...
namespace Vecerdi.Extensions.DependencyInjection.Infrastructure
{
    internal static partial class DependencyInjectionCache
    {
        ...
        public static void TestGenerated() { }
    }
}";

        context.AddSource("GeneratedTest.g.cs", source);
    ...
    private static string GenerateSource(ClassInfo[] classes)
    {
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        private static readonly ConcurrentDictionary<Type, IReadOnlyList<(PropertyInfo PropertyInfo, object? ServiceKey, bool IsRequired)>> s_GeneratedCache = new();");
        sb.AppendLine();
        ...
        {
            sb.AppendLine($"            // {classInfo.FullName}");
            sb.AppendLine($"            s_GeneratedCache[typeof({classInfo.FullName})] = new List<(PropertyInfo, object?, bool)>");
            sb.AppendLine("            {");
        ...
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        public static IReadOnlyList<(PropertyInfo PropertyInfo, object? ServiceKey, bool IsRequired)> GetInjectablePropertiesGenerated(Type type)");
        sb.AppendLine("        {");
        sb.AppendLine("            return s_GeneratedCache.TryGetValue(type, out var properties) ? properties : Array.Empty<(PropertyInfo, object?, bool)>();");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine("}");
